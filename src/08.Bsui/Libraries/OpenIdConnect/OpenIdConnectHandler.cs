...
#pragma warning restore CS8620 // Argument cannot be used for parameter due to differences in the nullability of reference types.
    ...
    private void SaveTokens(AuthenticationProperties properties, OpenIdConnectMessage message)
    {
        ...
        {
            if (int.TryParse(message.ExpiresIn, NumberStyles.Integer, CultureInfo.InvariantCulture, out var value))
            {
    ...
    private string? ReadNonceCookie(string nonce)
    {
        ...
        foreach (var nonceKey in Request.Cookies.Keys)
        {
            if (Options.NonceCookie.Name is string name && nonceKey.StartsWith(name, StringComparison.Ordinal))
            {
                try
                {
                    var nonceDecodedValue = Options.StringDataFormat.Unprotect(nonceKey[Options.NonceCookie.Name.Length..]);
...
#pragma warning disable CS0618 // Type or member is obsolete
        var principal = Options.SecurityTokenValidator.ValidateToken(idToken, validationParameters, out var validatedToken);
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    private async Task<TokenValidationResult> ValidateTokenUsingHandlerAsync(string idToken, AuthenticationProperties properties, TokenValidationParameters validationParameters)
    {
        ...
        if (!validationResult.IsValid || validatedToken == null)
        {
            ...
            throw new SecurityTokenException(string.Format(CultureInfo.InvariantCulture, "Unable to validate the 'id_token', no suitable TokenHandler was found for: '{0}'.", idToken));
        }

        if (validatedToken is not JsonWebToken)
        {
            Logger.InvalidSecurityTokenTypeFromHandler(validatedToken?.GetType());
            throw new SecurityTokenException(string.Format(CultureInfo.InvariantCulture, "The Validated Security Token must be of type JsonWebToken, but instead its type is '{0}'.", validatedToken?.GetType()));
        }
