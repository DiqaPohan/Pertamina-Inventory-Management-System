...
public partial class AddDraftCatalog
{
    ...
    //private SfAutoComplete<string, IdamanUsers>? _sfappbisuserwithkbo;
    ...
    //private async Task OnFilterbisuserwithkbo(FilteringEventArgs args)
    //{
    //    args.PreventDefaultAction = true;
    //    if (args.Text.Length >= 6)
    //    {
    //        var orWhere = WhereFilter.Or(new List<WhereFilter> {
    //        new () { Field = "displayName", Operator = "contains", value = args.Text, IgnoreCase = true },
    //        new () { Field = "email", Operator = "contains", value = args.Text, IgnoreCase = true }
    //    });
    //        var query = new Query().Where(orWhere);
    //        query = !string.IsNullOrEmpty(args.Text) ? query : new Query();
    //        await _sfappbisuserwithkbo.FilterAsync(_dataCountbisuserwithkbo, query);
    ...
    //private async Task OnFocusbisuserwithkbo()
    //{
    //    await _sfappbisuserwithkbo.ShowPopupAsync();
    //}
    ...
    private async Task OnValidSubmit(EditContext formContext)
    {
        ...
        {
            if (string.IsNullOrEmpty(_request.Application_Status))
            {
                _strHeadDialog = "Mohon untuk melengkapi Field Berikut";
                _strContentDialog = "Field Aplikasi Status Kosong";
                _strLinkDialog = "";
                Visibility = true;
            }
            else
            {
                ...
                if (response.Error is not null)
                {
                    ...
                    {
                        _strHeadDialog = "Pembuatan Draft Aplikasi Gagal";
                ...
                _isLoading = false;
                _strHeadDialog = "Pembuatan Draft Aplikasi Berhasil";
                _strContentDialog = "Nama Aplikasi " + _request.Application_Name + " Telah berhasil di tambah, dengan code aplikasi berikut " + response.Result!.Code_App + " dan menunggu persetujuan";
                _strLinkDialog = "ok";
                Visibility = true;
            }
        }
