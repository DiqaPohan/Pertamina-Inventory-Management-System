...
using Pertamina.SolutionTemplate.Bsui.Services.Authorization.IdAMan.Models.GetAuhtorizationInfo;
using Pertamina.SolutionTemplate.Bsui.Services.Authorization.IdAMan.Models.GetPositions;
...
public class IdAManAuthorizationService : IAuthorizationService
{
    ...
    public async Task<GetAuthorizationInfoResponse> GetAuthorizationInfoAsync(string positionId, string accessToken, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                var idAManApplication = restResponse.Data.First(m => m.Application.Id == _idAManAuthorizationOptions.ObjectId);
