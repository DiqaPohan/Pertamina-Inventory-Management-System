...
using Pertamina.SolutionTemplate.Domain.Events;
...
using Pertamina.SolutionTemplate.Infrastructure.Persistence.Common.Models;
...
public class SolutionTemplateDbContext : DbContext, ISolutionTemplateDbContext
{
    ...
    public SolutionTemplateDbContext(
        DbContextOptions<SolutionTemplateDbContext> options,
    ...
    protected SolutionTemplateDbContext(DbContextOptions options)
        : base(options)
    {
    }
            ...
            foreach (var property in entry.Properties)
            {
                ...
                if (property.Metadata.IsPrimaryKey())
                {
                    ...
                    if (propertyName == nameof(IHasKey.Id))
                    {
                        ...
                        {
                            var desiredGuid = System.Guid.NewGuid();
                            auditEntry.EntityId = desiredGuid;
                        }
                    }
                ...
                switch (entry.State)
                {
                    ...
                    case EntityState.Modified:
                        if (property.IsModified)
                        {
                            ...
                            if (propertyName is nameof(IDeletable.IsDeleted))
                            {
